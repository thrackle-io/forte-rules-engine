...
contract DiamondTest is RulesEngineCommon {
    ...
    function setUp() public {
        red = createRulesEngineDiamond(diamondOwner);
    ...
    function test_diamond_cannotAdd() public {
        _testDiamondCannotModify(FacetCutAction.Add);
    }

    function test_diamond_cannotRemove() public {
        _testDiamondCannotModify(FacetCutAction.Remove);
    }

    function test_diamond_cannotReplace() public {
    ...
    function test_diamond_cannotInitializeMoreThanOnce() public {
        vm.expectRevert("Already initialized");
        RulesEngineInitialFacet(address(red)).initialize(address(0xdead));
        // we confirm that the owner is still the original owner
        assertEq(ERC173Facet(address(red)).owner(), diamondOwner, "Rules Engine Diamond address mismatch");
    }
