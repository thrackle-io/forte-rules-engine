...
contract GasReports is GasHelpers, RulesEngineCommon {
    ...
    function setUp() public {
        ...
        // // OFAC Plus Min Transfer In Separate Rules
...
/**********  OFAC Prep functions to ensure warm storage comparisons **********/
    ...
    function testGasExampleOFACWithMinTransferSeparateRules() public endWithStopPrank() {
        vm.startPrank(USER_ADDRESS);
        _exampleContractGasReport(5, address(userContractFCPlusMin), "Using REv2 OFAC with min transfer gas report in separate rules"); 
    }
    ...
    function testGasExampleOFACWithMinTransferInOneRule() public endWithStopPrank() {
        vm.startPrank(USER_ADDRESS);
        _exampleContractGasReport(5, address(userContractFCPlusMinPlusMaxOneRule), "Using REv2 OFAC with min transfer gas report in one rule"); 
    }

    function testGasExampleOFACWithMinTransferInSeparatePolicies() public endWithStopPrank() {
        vm.startPrank(USER_ADDRESS);
        _exampleContractGasReport(5, address(userContractFCPlusMinSeparatePolicy), "Using REv2 OFAC with min transfer gas report in separate policies"); 
    }
    ...
    function setupRuleWithOFACForeignCall(
        ...
        rule.placeHolders[0].typeSpecificIndex = uint128(0); // hardcoded to 0 for now
        ...
        //     policyIds
        // );
        policy.trackers = new Trackers[](0);
        policy.rules = new Rule[](1);
        policy.rules[0] = rule;
        policy.foreignCalls = new ForeignCall[](1);
        policy.foreignCalls[0] = fc;
        policy.parameterTypes = pTypes;
        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFC), bytes4(keccak256(bytes(callingFunction))), policy);
    }
    ...
    function setupRulesWithForeignCallAndMinTransfer(
        ...
        rule1.placeHolders[0].typeSpecificIndex = uint128(0);
        ...
        policy.rules = new Rule[](2);
        policy.rules[0] = rule1;
        policy.rules[1] = rule2;
        // vm.stopPrank();
        // vm.startPrank(callingContractAdmin);
        // RulesEnginePolicyFacet(address(red)).applyPolicy(
        ...
        //     policyIds
        // );
        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFCPlusMin), bytes4(keccak256(bytes(callingFunction))), policy);
    }
   ...
   function setupRulesWithForeignCallPlusMinTransferAndMaxTransferInOneRule(
        ...
        rule1.placeHolders = new Placeholder[](2);
        rule1.placeHolders[0].foreignCall = true;
        rule1.placeHolders[0].typeSpecificIndex = uint128(0); // hardcoded to 0 for now
        rule1.placeHolders[1].pType = PT.UINT;
        rule1.placeHolders[1].typeSpecificIndex = 1;
        ...
        //     policyIds
        // // );
        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFCPlusMinPlusMaxOneRule), bytes4(keccak256(bytes(callingFunction))), policy);
    }
    ...
    function setupRulesWithForeignCallAndMinTransferSeparatePolicies(
        ...
        Policy memory policy1;
        Policy memory policy2;

        PT[] memory pTypes = new PT[](2);
        pTypes[0] = PT.ADDR;
        pTypes[1] = PT.UINT;
        
        policy1.parameterTypes = pTypes;
        policy2.parameterTypes = pTypes;
        ...
        policy1.rules = new Rule[](1);
        policy1.rules[0] = rule1;
        policy2.rules = new Rule[](1);
        policy2.rules[0] = rule2;

        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFCPlusMinSeparatePolicy), bytes4(keccak256(bytes(callingFunction))), policy1);
        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFCPlusMinSeparatePolicy), bytes4(keccak256(bytes(callingFunction))), policy2);
    }
   ...
   function setupRulesWithForeignCallPlusMinTransferAndMaxTransfer(
...

        {
            ...
            policy.rules = new Rule[](3);
            policy.rules[0] = rule1;
            policy.rules[1] = rule2;
            policy.rules[2] = rule3;
        }
        
        RulesEngineProcessorFacet(address(red)).writePolicyContract(address(userContractFCPlusMinPlusMax), bytes4(keccak256(bytes(callingFunction))), policy);
    }
