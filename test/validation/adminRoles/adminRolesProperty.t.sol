...
contract RulesEngineAdminRolesTest is RulesEngineCommon, RulesEngineAdminRolesFacet {
    ...
    function _initializeForPropertyTesting() internal {
        ...
        if (pfcContractAddress == address(0)) {
            pfcContractAddress = address(new PermissionedForeignCallTestContract());
            PermissionedForeignCallTestContract(pfcContractAddress).setRulesEngineAddress(address(red));
            functionSelector = PermissionedForeignCallTestContract(pfcContractAddress).simpleCheck.selector;
            PermissionedForeignCallTestContract(pfcContractAddress).setForeignCallAdmin(address(this), functionSelector);
        }
    }
    ...
    function property_AdminTransfersCorrectlyTracked() public ensureInitialized returns (bool) {
        ...
        // Foreign call admin transfers
        bytes32 foreignCallAdminRoleId = _generateForeignCallAdminRoleId(pfcContractAddress, functionSelector, FOREIGN_CALL_ADMIN);
        uint256 foreignCallAdminCount = RulesEngineAdminRolesFacet(address(red)).getRoleMemberCount(foreignCallAdminRoleId);

        // If we've had admin transfers, the counts should be consistent
        if (successfulPolicyAdminTransfers > 0 || successfulCallingContractAdminTransfers > 0 || successfulForeignCallAdminTransfers > 0) {
            // There should be at least 1 admin if transfers happened
            return (adminCount > 0 && ccAdminCount > 0 && foreignCallAdminCount > 0);
        }
    ...
    function property_AdminTransferSequenceIntegrity() public ensureInitialized returns (bool) {
        ...
        // For foreign call admins: proposals should always be >= successful transfers
        if (proposedForeignCallAdminCount < successfulForeignCallAdminTransfers) {
            return false;
        }
    ...
    /*//////////////////////////////////////////////////////////////
                  FOREIGN CALL ADMIN INVARIANTS
    //////////////////////////////////////////////////////////////*/
    // PROPERTY: Only the current Foreign Call admin can propose a new admin
    function property_OnlyCurrentForeignCallAdminCanProposeNewAdmin() public ensureInitialized returns (bool) {
        // If we've had unauthorized attempts, check that none succeeded
        if (unauthorizedActionAttempts > 0 || unauthorizedSuccessfulProposals > 0) {
            // Property holds if no unauthorized proposals succeeded AND
            // The number of successful proposals matches our counter
            bytes32 proposedRoleId = _generateForeignCallAdminRoleId(pfcContractAddress, functionSelector, PROPOSED_FOREIGN_CALL_ADMIN);
            uint256 actualProposedCount = RulesEngineAdminRolesFacet(address(red)).getRoleMemberCount(proposedRoleId);

            return unauthorizedSuccessfulProposals == 0 && proposedForeignCallAdminCount == actualProposedCount;
        }
        // If no unauthorized attempts yet, property holds
        return true;
    }

    // PROPERTY: Foreign Call can have at most one admin
    function property_ForeignCallAdminRoleHasOneAdmin() public ensureInitialized returns (bool) {
        bytes32 adminRoleId = _generateForeignCallAdminRoleId(pfcContractAddress, functionSelector, FOREIGN_CALL_ADMIN);

        uint256 adminCount = RulesEngineAdminRolesFacet(address(red)).getRoleMemberCount(adminRoleId);
        return adminCount == 1;
    }

    // PROPERTY: Proposed Foreign Call admin can't act until confirmed
    function property_ProposedForeignCallAdminCannotActUntilConfirmed() public ensureInitialized returns (bool) {
        bytes32 adminRoleId = _generateForeignCallAdminRoleId(pfcContractAddress, functionSelector, FOREIGN_CALL_ADMIN);
        bytes32 proposedAdminRoleId = _generateForeignCallAdminRoleId(pfcContractAddress, functionSelector, PROPOSED_FOREIGN_CALL_ADMIN);
    ...
    function action_proposeNewForeignCallAdmin(address sender, address newAdmin) public {
        ...
        bool isCurrentAdmin = false;
        if (RulesEngineAdminRolesFacet(address(red)).isForeignCallAdmin(pfcContractAddress, msg.sender, functionSelector)) {
            isCurrentAdmin = true;
        }
        ...
        if (isCurrentAdmin) {
            // Only impersonate if the original caller is authorized
            vm.startPrank(sender);
            try RulesEngineAdminRolesFacet(address(red)).proposeNewForeignCallAdmin(pfcContractAddress, newAdmin, functionSelector) {
                proposedForeignCallAdminCount++;
            } catch {}
            vm.stopPrank();
        } else {
            // For unauthorized callers, don't impersonate - try directly
    ...
    function action_confirmNewForeignCallAdmin(address sender) public {
        if (testUserContract == address(0)) return;
        ...
        vm.startPrank(sender);
        try RulesEngineAdminRolesFacet(address(red)).confirmNewForeignCallAdmin(pfcContractAddress, functionSelector) {
            // If successful, track the transfer
            successfulForeignCallAdminTransfers++;
