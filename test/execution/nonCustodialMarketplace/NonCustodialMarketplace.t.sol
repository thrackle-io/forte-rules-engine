...
contract NonCustodialMarketplaceTest is RulesEngineCommon {
    ...
    string[calldataArgSize] calldataArgs = ["from", "to", "tokenId", "operator"];
    mapping(string => uint) calldataArgNameToIndex;
    ...
    function setUp() public {
        ...
        // 0: from,, 1: to,, 2: tokenId,, 3: msg.sender
        calldataArgNameToIndex["from"] = 0;
        calldataArgNameToIndex["to"] = 1;
        calldataArgNameToIndex["tokenId"] = 2;
        calldataArgNameToIndex["operator"] = 3;
    }
    ...
    function test_nonCustodialMarketplace_addressAllowlist(
        ...
        vm.startPrank(policyAdmin);
        tradeAddressToCheckIndex = tradeAddressToCheckIndex % calldataArgSize; // ensure the index is within bounds
        string memory tradeAddressToCheck = calldataArgs[tradeAddressToCheckIndex]; // this will make sure that "tradeAddressToCheck" is either "from", "to", or "operator"
        _setupNaughtyListRuleForOperator(address(userContractFC), address(testContract2), tradeAddressToCheck, true); // we setup the rule with the fuzzed account to check
        ...
        bool willRevert = (isBadMarketplace && tradeAddressToCheckIndex == 3) || // we only expect a revert if the marketplace is a bad one and we are checking for the marketplace in the rule, or...
    ...
    function test_nonCustodialMarketplace_tokenIdAllowlist(bool isStolenNFT) public {
        testContract2 = new ForeignCallTestContractOFAC();
        vm.startPrank(policyAdmin);
        _setupNaughtyListRuleForOperator(address(userContractFC), address(testContract2), "tokenId", false); // we setup the rule with the fuzzed account to check
    ...
    function _setupNaughtyListRuleForOperator(
        ...
        uint256 foreignCallId;
        {
            ...
            fc.encodedIndices[0].index = calldataArgNameToIndex[_tradeAddressToCheck];
